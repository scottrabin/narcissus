        ...
        skip: function () {
            ...
            for (;;) {
                ...
                if (ch === '\n' && !this.scanNewlines) {
                    ...
                    this.cursor++;
		    // if the comment starts as /**, treat it as a DOC_COMMENT
		    var isDocComment = ( input[ this.cursor ] === '*' ), start = ++this.cursor;
		    ...
		    // if it was a doc comment, hold the comment for the next token
		    if( isDocComment ){
			this._cjsDocComments.push( input.substring( start, this.cursor - 2 ) );
//			console.log( "Found doc comment: " + this._cjsDocComments );
		    }
                } else if (ch === '/' && next === '/') {
                    this.cursor++;
                    for (;;) {
