...
Narcissus.lexer = (function() {
    ...
    Tokenizer.prototype = {
        ...
        skip: function () {
            ...
            for (;;) {
                ...
                if (ch === '\n' && !this.scanNewlines) {
                    this.lineno++;
                } else if (ch === '/' && next === '*' && input[this.cursor+1] !== '*') {
                    this.cursor++;
                    for (;;) {
	...
	/**
	 * CJSDoc comment lexer
	 */
	lexDocComment: function(ch) {
	    var token = this.token, input = this.source;
	    token.type = DOC_COMMENT;

	    do {
		ch = input[this.cursor++];
	    } while( !(ch === '*' && input[this.cursor] === '/') );

	    this.cursor++;
	    token.value = input.substring( token.start, this.cursor );
	},
        ...
        get: function (scanOperand) {
            ...
            if ((ch >= 'a' && ch <= 'z') || (ch >= 'A' && ch <= 'Z') || ch === '$' || ch === '_') {
                this.lexIdent(ch);
	    } else if ( ch === '/' && input[this.cursor] === '*' && input[this.cursor+1] === '*'){
		this.lexDocComment(ch);
